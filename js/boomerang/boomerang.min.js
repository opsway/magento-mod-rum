...
//Check in boomerang.js for the full (uncompressed version)
function formatBwLatency(a,b){if(a){bandwidth=">10Mb";pre_bw=b.bw/1000;if(pre_bw<=64){bandwidth="<64Kb"}else{if(pre_bw>64&&pre_bw<=512){bandwidth="64Kb-512Kb"}else{if(pre_bw>512&&pre_bw<=2048){bandwidth="512Kb-2Mb"}else{if(pre_bw>2048&&pre_bw<=6000){bandwidth="2Mb-6Mb"}else{if(pre_bw<=10000){bandwidth="6Mb-10Mb"}}}}}latency=">1000ms";if(b.lat<=50){latency="<50ms"}else{if(b.lat>50&&b.lat<=100){latency="50-100ms"}else{if(b.lat>100&&b.lat<=200){latency="100-200ms"}else{if(b.lat>200&&b.lat<=400){latency="200-400ms"}else{if(b.lat<=1000){latency="400-1000ms"}}}}}}else{latency="NA";bandwidth="NA"}return{latency:latency,bandwidth:bandwidth}}function trackOutboundLink(c,b,d){try{_gaq.push(["_trackEvent",b,d])}catch(a){}setTimeout(function(){document.location.href=c},100)};